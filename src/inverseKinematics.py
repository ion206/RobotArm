...
def getAngs(x, y, z, l1, l2):
    ...
    theta2 = math.acos((d**2 - l1**2 - l2**2)/(2*l1*l2)) #Law of Cosines
    theta1 = math.asin((l2*math.sin(theta2))/d) # Law of Sines
    theta1 = theta1 + math.asin(z/d)
    return (base_angle), int(math.degrees(theta1)), int(math.degrees(theta2))
